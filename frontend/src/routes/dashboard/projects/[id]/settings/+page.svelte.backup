<script lang="ts">
  ...
  let corsConfig: CORSConfig = {
    allowed_origins: ['http://localhost:3000', 'https://mijnapp.nl'],
  ...
  async function deleteKey(keyId: number) {
    if (confirm('Weet je zeker dat je deze API key wilt verwijderen?')) {
      try {
        const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/api-keys/${keyId}`, {
          method: 'DELETE',
          headers: {
      ...
      <div class="space-y-4">
        ...
        <div>
          ...
          <div class="mt-1 bg-muted p-3 rounded text-sm font-mono">
            ...
            <div class="ml-4 text-muted-foreground">http://localhost:8080/p/project-slug/api/data/users</div>
          </div>
        </div>
