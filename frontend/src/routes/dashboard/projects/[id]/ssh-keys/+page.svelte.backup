<script lang="ts">
  ...
  async function loadSSHKeys() {
    loading = true;
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/ssh-keys`, {
  ...
  async function deleteSSHKey(keyId: number) {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/ssh-keys/${keyId}`, {
        method: 'DELETE',
        headers: auth.getAuthHeader()
      });
