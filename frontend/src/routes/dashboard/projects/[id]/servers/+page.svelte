<script lang="ts">
  ...
  async function loadData() {
    ...
    try {
      ...
      // Load SSH keys
      const keysRes = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/ssh-keys`, { headers });
      if (keysRes.ok) {
  ...
  async function createWebServer() {
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/web-servers`, {
        method: 'POST',
        headers: {
  ...
  async function testConnection(serverId: number) {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/web-servers/${serverId}/test`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...auth.getAuthHeader()
  ...
  async function deleteWebServer(serverId: number) {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/web-servers/${serverId}`, {
        method: 'DELETE',
