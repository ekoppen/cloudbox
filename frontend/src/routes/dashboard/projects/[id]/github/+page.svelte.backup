<script lang="ts">
  ...
  async function loadRepositories() {
    loading = true;
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/github-repositories`, {
  ...
  async function syncRepository(repoId: number) {
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/github-repositories/${repoId}/sync`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...auth.getAuthHeader()
  ...
  async function deleteRepository(repoId: number) {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/github-repositories/${repoId}`, {
        method: 'DELETE',
        headers: auth.getAuthHeader()
      });
  ...
  async function showWebhookInfo(repo) {
    try {
      const response = await fetch(`http://localhost:8080/api/v1/projects/${projectId}/github-repositories/${repo.id}/webhook`, {
        headers: auth.getAuthHeader()
      });
