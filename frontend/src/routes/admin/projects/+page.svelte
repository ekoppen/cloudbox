<script lang="ts">
  ...
  async function loadProjects() {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/v1/admin/projects', {
        headers: {
  ...
  async function toggleProjectStatus(project: Project) {
    try {
      const response = await fetch(`http://localhost:8080/api/v1/admin/projects/${project.id}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${$auth.token}`,
          'Content-Type': 'application/json',
